...
- The active app contains only the 27 variant problems (`the-balance-lock` and `under-the-limit` were added on 2026-09-23 and are authored by `scripts/author-balance-lock.js` and `scripts/author-under-the-limit.js`). Original and new problems are archived: hide them from the picker and disable their lesson routes. Keep their source banks (25 original and 25 new) intact for future use and variant parent links. `activeCategories` in `visual-library.js` is the explicit archive switch.
