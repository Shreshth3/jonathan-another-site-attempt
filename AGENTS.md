...
- Step 3 is available at `/<problem-id>?section=3`. It shows three concise Yes/No claims and an exact graph build together on one screen.
- Step 3 claims must test node membership, direct edges versus reachability, and local degree. Show feedback only after submission so later choices cannot reveal earlier answers.
...
- Step 4 is available at `/<problem-id>?section=4`. It uses one deep, real problem-specific code-reasoning case per problem.
