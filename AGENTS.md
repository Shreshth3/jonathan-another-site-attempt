...
- The active app contains only the 26 variant problems (the 26th, `the-balance-lock`, was added on 2026-09-23 and is authored by `scripts/author-balance-lock.js`). Original and new problems are archived: hide them from the picker and disable their lesson routes. Keep their source banks (25 original and 25 new) intact for future use and variant parent links. `activeCategories` in `visual-library.js` is the explicit archive switch.
